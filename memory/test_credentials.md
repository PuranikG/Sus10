...
**NOTE:** Dev-only token. Created May 19, 2026 (iteration_15).
- Token: `iter15_test_session`
...
- Expires: 24 hours from creation (May 20, 2026).
- Usage:
  - HTTP: `curl -H "Authorization: Bearer iter15_test_session" $REACT_APP_BACKEND_URL/api/auth/me`
  - Playwright: set cookie `session_token=iter15_test_session` on domain `sus10-preview.preview.emergentagent.com`, path `/`, secure, sameSite Lax.
