...
**NOTE:** Dev-only token. Created May 20, 2026 (iteration_16).
- Token: `iter16_test_session` (iter15_test_session still valid until May 20)
...
- Expires: May 21, 2026.
- Usage:
  - HTTP: `curl -H "Authorization: Bearer iter16_test_session" $REACT_APP_BACKEND_URL/api/auth/me`
  - Playwright: set cookie `session_token=iter16_test_session` on domain `sus10-preview.preview.emergentagent.com`, path `/`, secure, sameSite Lax.
